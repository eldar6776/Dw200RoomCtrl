...
// No sqlite3 - all database operations via MQTT to device
...
// No local database - all operations via MQTT to device
...
// --- API Routes ---
// Note: All credential management (add/delete QR, PIN) is done via MQTT commands
// The web interface sends MQTT commands directly to the device
...
server.listen(port, () => {
    ...
    console.log('✓ Server ready - all operations via MQTT to device');
});

// No database functions - all operations forwarded via MQTT to device
