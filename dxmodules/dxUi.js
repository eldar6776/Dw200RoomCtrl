 ...
 * 1. Layers: device has 2 basic layers, main layer (main) and top layer (top)
      The TOP layer is always above the main layer. Switching pages on the main layer will not obscure the TOP layer. The TOP layer is suitable for displaying status bars.
      The main layer can construct multiple pages in memory in advance, and then use loadMain to load and switch different pages. The TOP layer cannot be switched, ui objects on it can only be hidden or deleted.
 ...
 * 2. UI objects: There are many types of UI objects, the most basic of which is the 'view' object. The root ui objects of the main layer and the top layer must be the 'view' object, and the remaining ui objects are all sub-uis of a certain ui object.
      UI objects include common ones such as 'button', 'label', 'image', etc. All objects have some common properties, and some unique properties.
      All UI objects have a globally unique id, which cannot be repeated. Common properties also include:
      - type: get the type of the ui object, string
      - parent: get the parent node of the ui object, string
      - children: get the ids of all child objects of the ui object, string array
